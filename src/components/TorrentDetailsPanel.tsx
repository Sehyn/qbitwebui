...
function InfoItem({
	...
	return (
		<div
			...
			style={{
				backgroundColor: 'color-mix(in srgb, white 2.5%, transparent)',
	...
	return (
		<div className="p-3 overflow-auto h-full">
			<div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
				<InfoItem
					label="Downloaded"
					value={formatSize(p.total_downloaded)}
					accent
				/>
				...
				<InfoItem
					label="Connections"
					value={`${p.nb_connections}/${p.nb_connections_limit}`}
				/>
...
function TrackersTab({ hash }: { hash: string }) {
	...
	if (isLoading) return <LoadingSkeleton />;
	const filtered =
		trackers?.filter((t: Tracker) => !t.url.startsWith('** [')) ?? [];
						...
						>
							...
							<td className="px-3 py-1.5 text-right">
								<div className="flex items-center justify-end gap-1.5">
									...
									<span
										className="font-mono w-7 text-right"
										style={{ color: 'var(--text-muted)' }}
									>
					...
					{flatNodes.map(({ node, depth }) => {
						...
						return (
							<tr
								...
								style={{ borderColor: 'var(--border)' }}
								onClick={
									node.isFolder ? () => toggleExpanded(node.path) : undefined
								}
							>
								<td className="px-3 py-1.5">
									<div
									...
									>
										<div
											className="shrink-0 w-3.5 h-3.5 rounded border transition-colors flex items-center justify-center cursor-pointer"
											style={{
											...
											onClick={(e) => {
												e.stopPropagation();
												// If skipped or mixed, toggle to normal (1). Otherwise toggle to skip (0).
												handlePriorityChange(
													node.fileIndices,
													isSkipped || isMixed ? 1 : 0,
												);
											}}
										>
											{isMixed ? (
										...
										{node.isFolder ? (
											<svg
												...
												style={{
													...
													transform: expanded.has(node.path)
														? 'rotate(90deg)'
														: 'rotate(0deg)',
												}}
												fill="none"
									...
									<div className="flex items-center justify-end gap-1.5">
										...
										>
											<div
												...
												style={{
													width: `${progress}%`,
													backgroundColor: done ? 'var(--accent)' : 'var(--text-muted)',
												}}
											/>
										</div>
										<span
								...
								>
									<select
										...
										onChange={(e) =>
											handlePriorityChange(node.fileIndices, parseInt(e.target.value))
										}
	...
	return (
		...
		>
			<div
				...
				className={`h-[3px] shrink-0 ${expanded
					? `cursor-ns-resize ${dragging ? 'opacity-100' : 'opacity-70 hover:opacity-100'}`
					: ''
					} transition-opacity`}
				style={{
					background:
